...
- **直接驱动 `evaluator-benchmark` 模块的 v0**:5 类能力 × 7 题型构成 kernel
  回归套件的最小子集。
...
- **3-stage × 4 control point 框架**与 kernel 内部 read 路径设计高度同构,
  `retriever-reranker` 输出 schema 应给 host 暴露 value / key / query 三个
  维度的元信息。
...
- 是首个把 chat-assistant memory 拆到"能力 × 题型"两维的 benchmark,概念
  分类清晰,对 memory kernel 的 schema 设计有立即指导价值。
...
- **统计学有效性问题**:正如 ConvoMem 所批评,500 题分到 7 类后,每类样本量
  在 30–150 之间,某些 sub-group 接近 ±40% 误差。回归时不应过度信任单
  类小数差异。
- LLM-as-Judge 评分有偏差,尤其在 KU / TR 类;PDF §4.3 也承认这点。实
  现 evaluator 时应保留人工抽检通道。
- benchmark 数据可能已被部分商用模型训练时见过(2025 年发布,2026 已经一年),
  评测时应监控对照组 vs LongMemEval-fresh 子集的差异。
...
- LongMemEval_M 的 1.5M token 子集对 host 侧 long-context model 的 latency /
  cost profile 影响,需在 host app 集成时实测。
- 中文 / 多语种扩展:host 侧的 zh-CN 场景需自行构造 LongMemEval-zh
  子集,目前 PDF 未提供。
