...
这一维上更精细,所以本仓的 taxonomy.md 把 `control-policy` 作为命名采纳的来
源(见 `taxonomy.md` §"2603.07670")。

三维 taxonomy:

- **Temporal-scope**:short-term / mid-term / long-term;并细分为 session-bound、
  task-bound、user-bound、persistent。直接对应 `valid_from / valid_to` 字段
  与 `supersedes` 关系。
...
本身不提出 benchmark,但其 §5 的横向对比可以直接作为 `evaluator-benchmark`
模块的 v0 选型矩阵:
...
§6 列出 9 个 open challenge,其中与 memory kernel 直接相关的 5 个:多模态 memory、long
...
- **temporal-scope** 维直接命中 `MemoryRecord` 的 `valid_from / valid_to /
  supersedes` 字段;这是 kernel 当前 schema 与该综述结构对齐的关键。
- **substrate** 维下的 hybrid 路径(vector + KG)正好对应 Mem0g(见
  `papers/mem0-paper.md`),host app 在 v1 可以借鉴。
- **control-policy** 维直接复用到 kernel 的 `dream-consolidator` 内部策略命名:
  kernel 不重新发明术语,采用 append / overwrite / supersede / consolidate /
  forget 五种 op,与 Mem0 的 ADD / UPDATE / DELETE / NOOP 形成一一映射后再扩展
  (consolidate 与 forget 是 Mem0 没有的两个 op)。
- 五机制族当中,kernel v0 主要落在 **Retrieval-augmented stores +
...
- §6 提到的"forgetting under regulation"与 mnemonic-sovereignty(2604.16548)
  应做一篇横向 ImpactNote,纳入 `security-privacy` 模块设计。
