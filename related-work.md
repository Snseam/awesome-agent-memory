...
1. **每条 ResearchItem 必须映射到 [`taxonomy.md`](taxonomy.md) 的 Ymem 模块**。
   别家是论文清单,我们是"驱动 Ymem 决策的输入面"。
2. **从知识 → ImpactReport → ADR → Ymem 主线** 是 awesome-list 的反操作。
   别家追求覆盖,我们追求"能进入 Ymem 设计"。
...
|---|---|---|---|---|
| `IAAR-Shanghai/Awesome-AI-Memory` | 上海 IAAR | 双语 zh-CN/en,综述式长 README(~590KB),Apache 2.0 | 他们做"百科";我们做"决策面" | zh-CN 词条命名;**中文综述读者画像** |
| `TsinghuaC3I/Awesome-Memory-for-Agents` | 清华 C3I 实验室 | `Persistence × Curation` 二维分类,月更,MIT | 他们的分类轴是论文学派;我们的轴是 Ymem 模块 | **Curation** 维度(write→manage→read 类比) |
...
| `qianlima-lab/awesome-lifelong-llm-agent` | qianlima-lab | TPAMI 2026 survey 配套,**lifelong** 视角 | lifelong / continual learning 与 Ymem `dream-consolidator` 强相关 | **catastrophic forgetting** 与 dream consolidation 的论文 lineage |
...
| `NirDiamant/Agent_Memory_Techniques` | Nir Diamant(个人)| 30 个 runnable Jupyter notebook,教程性质 | 我们不做教程,但**有 runnable code** 是宝贵的实证 | **notebook 实现作为 Ymem prototype 起点** |
...
这些边界本身也是 Ymem 的产品边界:Ymem 不是 RAG、不是 long-context、不是 persona,
是"agent 的状态持久与演进"。
